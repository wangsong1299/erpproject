...
def customer(request):
	...
	if not is_login:
		...
		for r in records:
			...
			a[i]=b
			i=i+1
		return render_to_response("customer_customer.html",{'is_login':json.dumps(is_login),'nick_name':nick_name,'records':a})
...
def supplier(request):
	...
	if not is_login:
		...
		for r in records:
			...
			a[i]=b
			i=i+1
		return render_to_response("customer_supplier.html",{'is_login':json.dumps(is_login),'nick_name':nick_name,'records':a})
...
def get_customerID_by_product(request):
	...
	id=Customer.objects.filter(product_name=product_name)[0].id
	return HttpResponse(id)
#api
@csrf_exempt
def get_customerID_by_customer(request):
	cumtomer = request.POST.get('customer', None)
	id=Customer.objects.filter(customers=cumtomer)[0].id
	return HttpResponse(id)
...
def get_supplierID_by_material(request):
	...
	id=Supplier.objects.filter(material_name=material_name)[0].id
	return HttpResponse(id)
#api
@csrf_exempt
def get_supplierID_by_supplier(request):
	suppliers = request.POST.get('supplier', None)
	id=Supplier.objects.filter(suppliers=suppliers)[0].id
	return HttpResponse(id)
